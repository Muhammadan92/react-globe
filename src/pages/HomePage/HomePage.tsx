...
import ImageWithDetails from 'jcicl/ImageWithDetails';
...
const HomePage = () => {
  ...
  const stopHover = (globeItem) => {
    ...
    if (globeItem?.properties?.ADMIN === 'United States of America' || globeItem?.properties?.ADMIN === 'Canada') return;
    if (globeItem) {
  ...
  const SampleImg = () => <div>
    <ImageWithDetails containerHeight='calc(100vh - 150px)' title={`${amount} Pounds of Food`} imageUrl={nigeriaFood} description={`Alhamdulillah, with the barakah of Shaykh Nurjan Mirahmadi ق, the blessed FZHH volunteers of ${location} have successfully rescued ${amount} pounds of food, saving it from being thrown away and delivering it into the hands of the hungry.`} imageAlt='Food Rescue Volunteers' />

    </div>
  const ImgCarousel = () =>       <Slider {...sliderSettings}>
  ...
  const CarouselWrapper = () => {
    return <CarouselContainer visible={carouselVisible}>
      <Button onClick={() => setCarouselVisible(false)} variant='icon' style={{ position: 'absolute', top: '5px', right: '5px'}}><CloseIcon /></Button>
      <SampleImg />
    </CarouselContainer>
  }
